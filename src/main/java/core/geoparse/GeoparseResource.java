...
public class GeoparseResource  extends AbstractServerResource {

    @Override
    protected Representation processGet(Representation representation) {
        String litUID = getAttribute("litUID");
        List<DBRow> table = DBConnection.getInstance().queryDB("SELECT * FROM `deepmap`.`geo_placename` WHERE linkUID = '"+litUID+"'");
