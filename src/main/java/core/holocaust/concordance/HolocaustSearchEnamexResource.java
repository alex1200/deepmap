 ...
 */
public class HolocaustSearchEnamexResource extends AbstractServerResource {
    ...
    protected Representation processPost(Representation representation) {
        try {
            ...
            if(null != search.getWord()) {
                concordanceList = ServerManager.getHoloEngine().searchEnamex(search.getWord(),10,false, search.getTextSelection());
            }
            else{
                concordanceList = ServerManager.getHoloEngine().searchEnamex(search.getLat(),search.getLon(),10,false, search.getTextSelection());
            }
